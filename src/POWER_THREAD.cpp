...
float	POWER::alpha 		= 0.850f;
float	POWER::power_detector 	= 0.0f;
float	POWER::power_threshold 	= 0.0005f;
bool	POWER::block_stored 	= false;
size_t	POWER::block_index 	= 0;
size_t	POWER::packet_num	= 0;

void POWER::power_thread
...
SharedPrinter& printer)
{
	...
	std::vector<std::complex<float>> buff (SIZE_BLOCKS_TO_STORE + 1,std::complex<float>(0.0f, 0.0f));
	...
	while(not stop_signal_called)
	{
		...
		for(size_t i = 0; i < fifo_output.size() ; i++)
		{
			...
			if(power_detector > power_threshold || block_stored == true)
			{
				...
				{
					//store to fifo
					buff[block_index+1] = fifo_output[i];
				...
				else
				{
					...
					{
						agc_sum = agc_sum +  std::norm(buff[1+i]);
					}
					float rms = std::sqrt(agc_sum / SIZE_BLOCKS_TO_STORE);
					for (int i = 0; i<SIZE_BLOCKS_TO_STORE ; i++)
                                        {
                                                buff[1+i] = buff[i+1]/rms;
                                        }
					std::cout << SIZE_BLOCKS_TO_STORE << std::endl;
	...
	std::cout << "Graceful exit of POWER_THREAD" << std::endl;
